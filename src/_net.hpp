...
template<class T>
void declare_bznetq(py::module &m, const std::string &typestr){
  using Class = BrillouinZoneNet3<T>;
  std::string pyclass_name = std::string("BZNetQ")+typestr;
  ...
  .def("__init__",[](BrillouinZone b, py::array_t<double> pypts, py::array_t<T> pydata, py::array_t<int, py::array::c_style> pyel){
    py::buffer_info bi;
    // copy over the predefined points:
    bi = pypts.request();
    if (bi.ndim != 2) throw std::runtime_error("points must be a 2-D array");
    if (bi.shape[1] != 3) throw std::runtime_error("points must be 3-vectors");
    Reciprocal lat = b.get_lattice();
    LQVec<double> qv(lat,(double*)bi.ptr, bi.shape, bi.strides); //memcopy
    // copy over the elements arrays
    bi = pyel.request();
    ...
    // copy over the data ArrayVector
    bi = pydata.request();
    ArrayVector<T> data((T*)bi.ptr, bi.shape, bi.strides);
    // let the constructor deal with everything else:
    return BrillouinZoneNet3<T>(b, qv, data, el);
  })
  ...
  .def("fill",[](Class& cobj, py::array_t<T> pydata, py::array_t<int, py::array::c_style> pyel){
    ...
    ArrayVector<T> data((T*)bi.ptr, bi.shape, bi.strides);
    //
    if (cobj.get_vertices().size() != data.size()){
      std::string msg = "Provided " + std::to_string(data.size())
                      + " data inputs but expected "
                      + std::to_string(cobj.get_vertices().size()) + "!";
