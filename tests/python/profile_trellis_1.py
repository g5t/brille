...
it = np.nditer([max_sizes, None, None, None],op_dtypes=('double','double','double','double'))
for m, s, i, ie in it:
    tictoc.tic()
    se = FibEu(nb, trellis=True, max_volume=m)
    s[...] = tictoc.toc()
    tictoc.tic()
    ...
    print('max volume {:f}: <time>={:5.3f}({:5.3f}) sum(time)={:5.2f} sec'.format(m,i,ie,tictoc.elapsed()))
...
pp.figure()
pp.errorbar(max_sizes, interpolation_time, yerr=interpolation_uncertainty, marker='o')
pp.xlabel(r'Maximum cell volume / $\AA^{-3}$')
pp.ylabel('interpolation of {} points / s'.format(q.shape[0]))
